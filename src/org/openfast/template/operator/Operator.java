...
public abstract class Operator {
	...
	protected static final Operator NONE_ALL = new AlwaysPresentOperator(NONE, Type.ALL_TYPES) {
	...
	protected static final Operator CONSTANT_ALL = new Operator(CONSTANT, Type.ALL_TYPES) {
		public ScalarValue getValueToEncode(ScalarValue value, ScalarValue priorValue, Scalar field) {
		...
		public ScalarValue decodeEmptyValue(ScalarValue previousValue, Scalar field) {
			if (!field.isOptional()) return field.getDefaultValue();
			return null;
		}
	...
	protected static final Operator DEFAULT_ALL = new Operator(DEFAULT, Type.ALL_TYPES) {
		public ScalarValue getValueToEncode(ScalarValue value, ScalarValue priorValue, Scalar field) {
			if (value == null) return ScalarValue.NULL;
...
//		}};
	...
	protected static final Operator INCREMENT_INTEGER = new Operator(INCREMENT, Type.INTEGER_TYPES) {
		...
		public ScalarValue decodeEmptyValue(ScalarValue previousValue, Scalar field) {
			if (previousValue == null || previousValue.isUndefined())
				if (field.getDefaultValue().isUndefined()) {
					...
					else throw new IllegalStateException("Field with operator increment must send a value if no previous value existed.");
				}
				else
					return field.getDefaultValue();
	...
	public static final Operator DELTA_INTEGER = new AlwaysPresentOperator(DELTA, Type.INTEGER_TYPES) {
		public ScalarValue getValueToEncode(ScalarValue value, ScalarValue priorValue, Scalar field) {
			if (priorValue == null) throw new IllegalStateException(ERR_D9);
			if (value == null)
				if (field.isOptional())
					return ScalarValue.NULL;
				else
		...
		public ScalarValue decodeValue(ScalarValue newValue, ScalarValue previousValue, Scalar field) {
			...
			if (newValue == null || newValue.isNull()) return null;
			if (previousValue.isUndefined()) 
				if (field.getDefaultValue().isUndefined())
	...
	public static final Operator DELTA_STRING = new DeltaStringOperator(DELTA, new Integer[] { Type.STRING });
				
	protected static final Operator DELTA_DECIMAL = new DeltaDecimalOperator();

	protected static final Operator TAIL_STRING = new Operator("tail", new Integer[] { Type.STRING }) {
		public ScalarValue getValueToEncode(ScalarValue value, ScalarValue priorValue, Scalar field) {
			if (value == null) return ScalarValue.NULL;
			if (priorValue == null) return value;
			if (priorValue.isUndefined())
				if (value.equals(field.getDefaultValue()))
					return null;
				else
					return value;
			int index = 0;
			for (; ((StringValue)value).value.charAt(index) == ((StringValue) priorValue).value.charAt(index); index++);
			return new StringValue(((StringValue)value).value.substring(index));
		}
		public ScalarValue decodeValue(ScalarValue newValue, ScalarValue previousValue, Scalar field) {
			StringValue base;
	...
	public static Operator getOperator(String name, Integer type)
	{
		...
		if (!OPERATOR_MAP.containsKey(key))
			throw new IllegalArgumentException("The operator \"" + name + "\" does not exist for the type specified.");
