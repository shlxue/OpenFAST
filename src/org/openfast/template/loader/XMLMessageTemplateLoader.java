...
public class XMLMessageTemplateLoader implements MessageTemplateLoader {
	private static final List NON_FIELD_ELEMENTS = Arrays.asList(new String[] { "messageRef", "length" });
	
	private ErrorHandler errorHandler = ErrorHandler.DEFAULT;

	public MessageTemplate[] load(InputStream source) {
		Document document = parseXml(source);
		if (document == null) return new MessageTemplate[] {};
		NodeList templateTags = document.getDocumentElement().getElementsByTagName("template");
		...
		for (int i=0; i<templateTags.getLength(); i++) {
			...
			templates[i] = new MessageTemplate(templateTag.getAttribute("name"), parseFields(templateTag));
	...
	private String getMessageReference(Element templateTag) {
		...
		NodeList messageReferenceTags = templateTag.getElementsByTagName("messageRef");
	...
	private Field parseField(Element fieldNode) {
		...
		if (fieldNode.getNodeName().equals("decimal")) {
			...
			if (mantissaNode != null || exponentNode != null)
				return createTwinFieldDecimal(fieldNode,name, optional, mantissaNode, exponentNode);
		}
		return createScalar(fieldNode, name, optional, fieldNode.getNodeName());
	}

	private Field createTwinFieldDecimal(Element fieldNode, String name, boolean optional, Node mantissaNode, Node exponentNode) {
		...
		if (mantissaNode != null && mantissaNode.hasChildNodes()) {
			...
			if (value != null && !value.equals(""))
				mantissaDefaultValue = ScalarValue.getValue(Type.UNSIGNED_INTEGER, value);
		}
		if (exponentNode != null && exponentNode.hasChildNodes()) {
			...
			String value = ((Element) operatorNode).getAttribute("value");
			if (value != null && !value.equals(""))
				exponentDefaultValue = ScalarValue.getValue(Type.UNSIGNED_INTEGER, value);
		}
		
		return new Scalar(name, Type.DECIMAL, new TwinOperator(exponentOperator, mantissaOperator), new TwinValue(mantissaDefaultValue, exponentDefaultValue), optional);
	}
	...
	private Scalar createScalar(Element fieldNode, String name, boolean optional, String typeName) {
		...
		if (operatorElement.hasAttribute("value"))
			return new Scalar(name, typeName, operatorElement.getNodeName(), operatorElement.getAttribute("value"), optional);
		return new Scalar(name, typeName, operatorElement.getNodeName(), optional);
	}

	private Element getOperatorElement(Element fieldNode) {
		NodeList children = fieldNode.getChildNodes();
		int index = 0;
		for (Node item = children.item(0); index<children.getLength(); item = children.item(index++))
		{
			if (isElement(item))
			{
				return ((Element) item);
	...
	private Sequence parseSequence(Element sequence, boolean optional) {
		return new Sequence(getName(sequence), parseSequenceLengthField(sequence, optional), parseFields(sequence), optional);
	}
	...
	private Scalar parseSequenceLengthField(Element sequence, boolean optional) {
		...
		if (lengthElements.getLength() == 0) return null;
		Element length = (Element) lengthElements.item(0);
		String name = length.hasAttribute("name") ? length.getAttribute("name") : Sequence.createUniqueName();
	...
	private Document parseXml(InputStream templateStream) {
		try {
			...
			errorHandler.error(XML_PARSING_ERROR, "Error occurred while parsing xml template.", e);
		} catch (FactoryConfigurationError e) {
			errorHandler.error(XML_PARSING_ERROR, "Error occurred while parsing xml template.", e);
		} catch (SAXException e) {
			errorHandler.error(XML_PARSING_ERROR, "Error occurred while parsing xml template.", e);
		} catch (IOException e) {
			errorHandler.error(IO_ERROR, "Error occurred while trying to read xml template.", e);
		}
		return null;
	}
	...
	private static final ErrorCode IO_ERROR = new ErrorCode(FastConstants.STATIC, -1, "IOERROR", "IO Error", FastAlertSeverity.ERROR);
	private static final ErrorCode XML_PARSING_ERROR = new ErrorCode(FastConstants.STATIC, -1, "XMLPARSEERR", "XML Parsing Error", FastAlertSeverity.ERROR);
}
