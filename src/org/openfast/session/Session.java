...
public class Session implements ErrorHandler {
	...
	private final static MessageTemplate FAST_ALERT_TEMPLATE = new MessageTemplate("", new Field[] {
			new Scalar("Severity", Type.UNSIGNED_INTEGER, Operator.NONE, false),
			new Scalar("Code", Type.UNSIGNED_INTEGER, Operator.NONE, false),
			new Scalar("Value", Type.UNSIGNED_INTEGER, Operator.NONE, ScalarValue.UNDEFINED, true),
			new Scalar("Description", Type.STRING, Operator.NONE, false),
		});
	
	private final static MessageTemplate FAST_RESET_TEMPLATE = new MessageTemplate("", new Field[] {});
	
	private final static MessageTemplate FAST_HELLO_TEMPLATE = new MessageTemplate("", new Field[] {
			new Scalar("SenderName", Type.STRING, Operator.NONE, false)
		});
	
	public static final Message RESET = new Message(FAST_RESET_TEMPLATE, FAST_RESET_TEMPLATE_ID) {
	...
	public Session(InputStream inputStream, OutputStream outputStream) {
		...
		MessageHandler resetHandler = new MessageHandler() {
			public void handleMessage(GroupValue readMessage, Context context, Coder coder) {
				coder.reset();
			}};
	...
	public static Message createHelloMessage(String name) {
		Message message = new Message(FAST_HELLO_TEMPLATE, FAST_HELLO_TEMPLATE_ID);
	...
	public void setErrorHandler(ErrorHandler errorHandler) {
		if (errorHandler == null) this.errorHandler = ErrorHandler.NULL;
