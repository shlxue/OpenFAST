...
public class FastEncoderTest extends TestCase {
	...
	public void testEncodeSequentialEmptyMessages()
	{
		...
		TestUtil.assertBitVectorEquals("11000000 11110001", encoder.encode(message));
	...
	public void testEncodeSimpleMessage()
	{
		...
		FastEncoder encoder = new FastEncoder(context);
		TestUtil.assertBitVectorEquals("11100000 11110001 10000001", encoder.encode(message));
	}
	...
	public void testEncodeMessageWithAllFieldTypes()
	{
		...
		TestUtil.assertBitVectorEquals(msgstr, new FastEncoder(context).encode(message));
	}
	
	public void testEncodeMessageWithOverlongPmap() {
		
		MessageTemplate template = new MessageTemplate(null, new Field[] {
				new Scalar("1", Type.UNSIGNED_INTEGER,Operator.COPY, new IntegerValue(1), false),
				new Scalar("1", Type.UNSIGNED_INTEGER,Operator.COPY, new IntegerValue(1), false),
				new Scalar("1", Type.UNSIGNED_INTEGER,Operator.COPY, new IntegerValue(1), false),
				new Scalar("1", Type.UNSIGNED_INTEGER,Operator.COPY, new IntegerValue(1), false),
				new Scalar("1", Type.UNSIGNED_INTEGER,Operator.COPY, new IntegerValue(1), false),
